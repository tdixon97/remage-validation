...
def replace_lines(input_file: str, output_file: str, replacements: dict[str, str]) -> None:
    """
...
def run_sim(generator_name = "",name = "",val = "0",step_limits = "",prod_cuts = "",step_points= "",generator="",register_lar = False):
    macro_file = "mac.mac"
    ...
    lar_command = "/RMG/Geometry/RegisterDetector Scintillator LAr 002" if register_lar else ""

    stp_directory.mkdir(parents=True, exist_ok=True)
    macro_directory.mkdir(parents=True, exist_ok=True)

    replacements = {"$STEP_LIMITS_COMMAND":step_limits,
                    "$PROD_CUTS_COMMAND":prod_cuts,
                    "$GENERATOR":generator,
                    "$STEP_POINT":step_points,
                    "$REGISTER_LAR": lar_command
    }
    replace_lines('macros/template.mac', macro_directory / Path(macro_file), replacements)

    start = time.time()
    result = subprocess.run(f"remage {macro_directory/ macro_file} -g gdml/geometry.gdml -o {stp_directory}/out.lh5 -w  ", shell=True)
    end = time.time()
...
for generator, config in generators.items():
    ...
    profile[generator]= {"def_lar_on":{},"def_lar_off":{},"step_limits":{},"sens_prod_cuts":{},"def_prod_cuts_lar_on":{},"def_prod_cuts_lar_off":{}}
